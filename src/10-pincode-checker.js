...
export function isValidParcelWeight(weight) {
  // Your code here
  if(Number.isNaN(weight)||weight === Infinity ||typeof weight !=="number"||weight<=0||!Number.isFinite(weight))return false;
  return true;
...
export function isWholeNumber(value) {
  // Your code here
  if(Number.isNaN(value)||typeof value !=="number"||value === Infinity)return false;
